...
function HomeHeader({user, selected}){
    ...
    {/* collect data from firebase(database) */}
    useEffect(() => {
        db.collection('users').onSnapshot(snapshot => {
            //This statement over here gets all the users from the collection irrespective of the search term.
            setUsers(snapshot.docs.map((doc) => doc.data()))
        })
        if( users !== undefined ){
            const finalUsers = users.filter(user => {
                return user.displayName.toLowerCase().indexOf(searchTeam.toLowerCase()) !== -1;
            })
        ...
        db.collection('posts').onSnapshot((snapshot) => {
            snapshot.docs.map((doc) => {
                if(user && doc.data().userName === user.displayName){
                    // Checking for only the currently logged
    ...
    return(
        <div className="homeHeader">
          ...
          <div className="homeHeaderLogoAndSearch">
              <Link to="/">
                  <img src="https://i.ibb.co/72dN4JJ/Facebook-icon-2019-1.png" class="homeHeader_logo" />
              </Link>
              {/* for searchbox */}
              ...
              <div className="home">
                  {
                      selected ? (
                          <Link to="/">
                          {/*BootStrap */}
                             <svg viewBox="0 0 28 28" class={`PagesIcon ${selected && "selected"}`} height="28" width="28">
                        ...
                        <Link to="/">
                          <svg viewBox="0 0 28 28" class="PagesIcon" height="28" width="28">
